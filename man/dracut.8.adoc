...
**--hostonly-cmdline**::
    ...
    priority over initrd arguments generated by dracut modules.
    For example the _rootfs-block_ dracut module could store the _root=_
    initrd cmdline argument in the _etc/cmdline.d_ directory, which would
    set the root device if it is not already specified in _/proc/cmdline_.

**--no-hostonly-cmdline**::
    Do not store kernel command line arguments needed in the initramfs.
    Ensures that dracut modules are not allowed to generate and store
    host-specific initrd arguments in the _etc/cmdline.d_ directory inside the
    generated initrd.
...
and command-line options override both.
